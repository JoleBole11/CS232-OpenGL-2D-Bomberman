...
Button::Button(glm::vec2 position, glm::vec2 scale, string text, Sprite* fontSprite, glm::vec3 buttonColor) {
	...
	_textRenderer = new TextRenderer(glm::vec2(position.x - 16 * (text.size() + 1), get_position().y - 16),
		new Sprite("Sprites/font.png", glm::vec2(32), 1, glm::vec2(15, 8), true), 27, 32);
	_textRenderer->setText(text);
...
void Button::render() {
	...
	glTranslatef(get_position().x, get_position().y, 0.0f);
	glRotatef(get_rotation(), 0.0f, 0.0f, 1.0f);
	glScalef(get_scale().x, get_scale().y, 1.0f);
